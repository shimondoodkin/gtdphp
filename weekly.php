...
//PAGE DISPLAY CODE
	...
	echo '</TR>';
	echo '<tr><td>Gather all loose papers</td><td></td></tr>';
	echo '<tr><td>Process all notes</td><td></td></tr>';
	echo '<tr><td>Check all voice mail</td><td></td></tr>';
	echo '<tr><td>Review Email Inbox </td><td>Move each item to an action, waiting, or reference folder.</td></tr>';
	echo '<tr><td>Review Email action box </td><td>Put next action reminder into system for each one, delete completed emails or move to waiting on or reference boxes as approriate.</td></tr>';
	echo '<tr><td>Review Email waiting on box </td><td>Put waiting on reminder into system for each one, delete completed emails or move to reference box as approriate.</td></tr>';
	echo '<tr><td>Review previous calendar</td><td>Transfer any missed actions to system</td></tr>';
	echo '<tr><td>Purge agendas</td><td></td></tr>';
	echo '<tr><td>Review upcoming calendar</td><td>Capture actions about arrangements and preparations for any upcoming events</td></tr>';
	echo '<tr><td>Empty your head</td><td>Put in writing any new <a href="project.php?type=p" title="Add project">projects</a>, <a href="item.php?type=a" title="Add action">actions</a>, <a href="item.php?type=w" title="Add waitingOn">waitingOn</a>, <a href="item.php?type=r" title="Add reference">references</a>, and <a href="project.php?type=s" title="Add Someday/Maybe">someday/maybes</a> that are not yet in the system.</td></tr>';
	echo '<tr><td>Review <a href="listProjects.php?pType=p">Projects list</a></td><td>Evaluate status of each project, goals, outcomes, one by one, ensuring that at least one next action exists for each';
